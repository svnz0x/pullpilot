...
- Si necesitas volver a copiar los archivos de `config.defaults` (añade nuevos ficheros o directorios y mantenlos sincronizados con `config` sin pisar tus personalizaciones) ejecuta `python -m pullpilot.cli.sync_defaults --target /ruta/a/config.defaults` o el entrypoint equivalente `pullpilot-sync-defaults`.
...
- **Permisos del fichero del token**: el archivo apuntado por `PULLPILOT_TOKEN_FILE` debe ser un fichero regular y no puede ser escribible/ejecutable por el grupo u otros usuarios. Los modos `0600`, `0640`, `0644`, `0440` o `0444` son válidos (cualquier combinación equivalente donde solo el propietario tenga permisos de escritura). Si los permisos son más laxos, el backend registrará una advertencia y descartará el archivo.
